...
function createDoomscrollTask(configuration: InstagramPluginConfiguration): TaskDefinition<DoomscrollPayload> {
    return {
        type: 'doomscroll', version: 1, displayName: 'Instagram doomscroll',
        validate(value) {
...
function createPostTask(configuration: InstagramPluginConfiguration): TaskDefinition<PostPayload> {
    return {
        ...
        validate(value, context) {
            ...
            if (typeof input.account !== 'string' || !input.account.trim()) throw new Error('Choose an Instagram account');
        ...
        async registerRoutes(context) {
                    ...
                    try {
                        if (kind === 'now') {
                            ...
                            if (mine.some(({ status }) => status === 'running')) {
                                throw new Error('An Instagram doomscroll is already running on this device. Stop it from Activity, then start again.');
                            }
            ...
            context.app.post<{ Params: { udid: string } }>('/api/devices/:udid/instagram/posts', async (request, reply) => {
                ...
                try {
                    ...
                    if (!account) throw new Error('Choose an Instagram account');
